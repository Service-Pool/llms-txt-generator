...
export class Migration1768035449550 implements MigrationInterface {
    name = 'Migration1768035449550'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`generations\` (\`id\` int UNSIGNED NOT NULL AUTO_INCREMENT, \`hostname\` varchar(500) NOT NULL, \`provider\` enum ('gemini', 'ollama') NOT NULL, \`status\` enum ('calculated', 'waiting', 'active', 'completed', 'failed') NOT NULL DEFAULT 'waiting', \`content\` longtext NULL, \`error_message\` text NULL, \`entries_count\` int UNSIGNED NULL, \`calculation_id\` int UNSIGNED NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`idx_hostname\` (\`hostname\`), UNIQUE INDEX \`unique_generation\` (\`hostname\`, \`provider\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
