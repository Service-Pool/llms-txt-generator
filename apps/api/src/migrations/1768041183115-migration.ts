...
export class Migration1768041183115 implements MigrationInterface {
    name = 'Migration1768041183115'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`generation_requests\` (\`id\` int NOT NULL AUTO_INCREMENT, \`generation_id\` int UNSIGNED NOT NULL, \`user_id\` int UNSIGNED NULL, \`session_id\` varchar(128) NULL, \`payment_link\` varchar(500) NULL, \`is_paid\` tinyint NOT NULL DEFAULT 0, \`requested_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), INDEX \`idx_generation\` (\`generation_id\`), INDEX \`idx_user\` (\`user_id\`), INDEX \`idx_session\` (\`session_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`generations\` (\`id\` int UNSIGNED NOT NULL AUTO_INCREMENT, \`calculation_id\` int UNSIGNED NOT NULL, \`provider\` enum ('gemini', 'ollama') NOT NULL, \`status\` enum ('calculated', 'pending-payment', 'waiting', 'active', 'completed', 'failed') NOT NULL DEFAULT 'waiting', \`content\` longtext NULL, \`error_message\` text NULL, \`entries_count\` int UNSIGNED NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`unique_generation\` (\`calculation_id\`, \`provider\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
