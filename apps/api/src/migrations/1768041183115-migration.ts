...
export class Migration1768041183115 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`generations\` (\`id\` int UNSIGNED NOT NULL AUTO_INCREMENT, \`calculation_id\` int UNSIGNED NOT NULL, \`provider\` enum ('gemini', 'ollama') NOT NULL, \`status\` enum ('calculated', 'waiting', 'active', 'completed', 'failed') NOT NULL DEFAULT 'waiting', \`content\` longtext NULL, \`error_message\` text NULL, \`entries_count\` int UNSIGNED NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`unique_generation\` (\`calculation_id\`, \`provider\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
