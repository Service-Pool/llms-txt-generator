...
export class Migration1778425178283 implements MigrationInterface {
    name = 'Migration1778425178283'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`orders\` (\`id\` int NOT NULL AUTO_INCREMENT, \`userId\` int NULL, \`sessionId\` varchar(255) NULL, \`hostname\` varchar(255) NOT NULL, \`modelId\` varchar(255) NULL, \`strategy\` enum ('flat', 'clustered') NOT NULL, \`priceTotal\` decimal(11,6) NULL, \`priceCurrency\` enum ('EUR') NULL, \`pricePerUrl\` decimal(11,6) NULL, \`stripeSessionId\` varchar(255) NULL, \`stripePaymentIntentSecret\` varchar(255) NULL, \`status\` enum ('created', 'calculated', 'pending_payment', 'paid', 'payment_failed', 'queued', 'processing', 'completed', 'failed', 'cancelled', 'refunded') NOT NULL DEFAULT 'created', \`jobId\` varchar(255) NULL, \`urlsTotal\` int NULL, \`progress\` json NULL, \`startedAt\` datetime NULL, \`completedAt\` datetime NULL, \`output\` longtext NULL, \`urlListFilter\` varchar(500) NULL, \`urlsFiltered\` int NULL, \`urlList\` longtext NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
