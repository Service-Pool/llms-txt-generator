 ...
 * Gemini LLM service
 *
 * Implements AI summarization using Google Gemini API.
...
export class GeminiService implements AiServiceInterface {
	...
	public constructor(private readonly configService: AppConfigService) {
		this.ai = new GoogleGenAI({ apiKey: configService.gemini.apiKey });
	}
	 ...
	 * 4. Send single request to Gemini with JSON schema
	...
	public async generatePageSummaries(summaries: UrlSummary[]): Promise<void> {
		...
		try {
			response = await this.generate(
				batchPrompt,
				this.configService.gemini.model,
				{
					temperature: this.configService.gemini.temperature,
					maxOutputTokens: this.configService.gemini.maxTokens,
	...
	/**
	 * Low-level wrapper for Gemini API calls
	 *
	 * Handles:
	 ...
	 * @param config - Optional generation configuration (temperature, tokens, schema, etc.)
	 * @returns Generated text response from LLM
	 * @throws Error if API call fails or returns empty response
	 */
	private async generate(prompt: string, model: string, config?: GenerateContentConfig): Promise<string> {
